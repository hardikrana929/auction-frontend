...
const baseLinks = [
  ...
  { to: "/auctions", label: "Auctions", icon: FiActivity },
  { to: "/live-auctions", label: "Live Auctions", icon: FiActivity },
...
export default function DashboardLayout() {
  ...
  const links = isAdmin
    ? [
        ...
        { to: "/admin/teams", label: "Team Management", icon: FiUser },
        { to: "/admin/players", label: "Player Management", icon: FiUser },
        { to: "/admin/registrations", label: "Registrations", icon: FiBell },
        {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div
        className={`fixed inset-y-0 left-0 z-40 w-72 transform border-r border-slate-200 bg-white transition-transform dark:border-slate-800 dark:bg-slate-900 lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"} ${collapsed ? "lg:w-20" : ""}`}
      >
        ...
        <nav className="space-y-1 p-3">
          {links.map(({ to, label, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition ${isActive ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"} ${collapsed ? "lg:justify-center" : ""}`
              }
            >
        ...
        <div className="absolute bottom-0 w-full border-t border-slate-200 p-3 dark:border-slate-800">
          <button
            onClick={doLogout}
            className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 ${collapsed ? "lg:justify-center" : ""}`}
          >
            <FiLogOut />
            {!collapsed && "Logout"}
          </button>
