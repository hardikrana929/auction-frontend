...
export default function TeamManagement() {
  ...
  const toggleStatus = async (team) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Unable to update team status.",
      );
    }
  };
      ...
      <section className="page-container">
        <div className="mb-6">
          ...
          <p className="page-subtitle">
            Manage the team identity and ownership. Purse values remain
            backend-controlled.
          </p>
        </div>
    ...
    <section className="page-container">
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="page-subtitle">
            Create, review and manage teams registered for each auction.
          </p>
        </div>
        <button
      ...
      {error ? (
        ...
        <EmptyState
          ...
          description="Choose an auction to view its teams."
        />
      ) : filtered.length === 0 ? (
        <EmptyState
          icon={<FiUsers />}
          ...
          description={
            search
              ...
              : "Create the first team for this auction."
          }
        />
      ) : (
      ...
      {selected && (
        ...
        >
          <div
            className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-3xl bg-white p-6 dark:bg-slate-900 sm:rounded-3xl"
          ...
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <Detail
                ...
                value={selected.remainingBudget}
              />
              <Detail
                label="Players"
                value={
                  Array.isArray(selected.players) ? selected.players.length : 0
                }
              />
            </div>
          </div>
...
function Detail({ label, value, mono }) {
  return (
    <div className="flex items-start justify-between gap-4 rounded-xl bg-slate-50 p-3 dark:bg-slate-800">
      ...
      <span
        className={`max-w-[65%] break-all text-right font-semibold text-slate-900 dark:text-white ${mono ? "font-mono text-xs" : ""}`}
      >
        {value}
      </span>
    </div>
